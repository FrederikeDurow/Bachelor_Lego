...
import UR_Config as rtde_config
import struct
import socket
import select
import sys
...
class RTDE(object):
    ...
    def Robot_Host_set (self, input):
        ROBOT_HOST = input

    def Robot_Port_set (self, input):
        ROBOT_PORT = input

    def Config_file_set(self, str):
        config_filename = str

    def Load_URP_script(self, str):
        if(str.endswith('.urp')):
            URP_script = str
        else:
...
# Start is in utilities
# Pause is in utilities
# end is in utilities
